...
def parse_option():
    ...
    parser.add_argument('--cfg', type=str, default='./cfgs/ca_deit_t.yaml', metavar="FILE", help='path to config file', )
    parser.add_argument(
        "--opts",
        help="Modify config options by adding 'KEY VALUE' pairs. ",
        default=['MODEL.NAME', 'example',
                 'DATA.IMG_SIZE', reso,
                 # 'MODEL.SWIN.NUM_HEADS', '[3, 3, 3, 12]'
                 ],
        nargs='+',
    )
...
if __name__ == '__main__':
    ...
    # ---------- 可视化等 ----------
    # model.load_state_dict(torch.load('./ckpt_epoch_22.pth', map_location='cpu')['model'])
    # model.eval()
